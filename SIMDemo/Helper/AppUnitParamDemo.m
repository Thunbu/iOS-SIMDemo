...
#import "AppUnitParamDemo.h"
#import <sys/utsname.h>

@implementation AppUnitParamDemo

+ (BOOL)isIphoneX {
    NSString * iphoneType = [AppUnitParamDemo getIphoneType];
...
+ (void)systemHomeIconBadgeWithAdd:(BOOL)add sub:(BOOL)sub euqal:(BOOL)equal much:(NSInteger)much {
    dispatch_async(dispatch_get_main_queue(), ^(void) {
        NSInteger targetNum = [UIApplication sharedApplication].applicationIconBadgeNumber;
...
+ (NSString *)uniqueId {
    ...
    NSString *clientId = (__bridge NSString *) string;
    CFRelease(theUUID);
    CFRelease(string);
    return clientId;
}
...
+ (NSString *)appDisplayName {
    NSString *displayNameString = [[NSBundle bundleForClass:[self class]] infoDictionary][ @"CFBundleDisplayName" ];
